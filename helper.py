...
def filename(item, item2):
    ...
    return 'temp_data_json/{}_{}.json'.format("_".join(item['Name'].replace("/", " ").split(' ')).lower(), item2['Name'])
...
def json_to_csv(dir = 'temp_data_json'):
    json_dir = os.listdir(dir)
    for json_file in json_dir:
        
        json_data = pd.read_json('temp_data_json/' + json_file)
        json_data.to_csv('temp_data_csv/{}.csv'.format(json_file.split('.')[0]))

def remove_json_temp(folder = 'temp_data_json'):
