...
sample_loc_lookup <- read.csv("bldg_to_sampleloc.csv") %>% 
  ...
  filter(!building %in% c("MSU FCU1", "MSU FCU2", paste0("Veterinary Medical Center", 1:5)))
...
excluded_bldgs <- c(1802:1810, "Engineering Research", paste0("FRIB", 1:8), "Observatory", "Plant Biology Laboratories2")
...
# Combine all listed dataframes into one
wm_daily_dat <- df_list %>% reduce(full_join, by='date') %>% 
